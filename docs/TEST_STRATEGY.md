...
**Phase U1 (2026-04-18):** `tests/test_workbench_u1.py` — workbench evidence panel (session project required), POST assist renders citations + provenance; legacy vs explicit projection visible in HTML; governance conflict refusal surfaced; citation source route returns fragment text and (when available) original-file context; deterministic hooks section distinguished from citations; missing session redirects to hub.

**Phase U2 (2026-04-18):** `tests/test_workbench_u2.py` — chat-first routes (`/workbench/project/chat`, `POST .../chat/query`) mirror U1 orchestrator semantics; empty page has no assist article; hub lists chat before workflow; citations + authority + refusal + normative vs supporting badge classes; fragment links resolve; missing/invalid project session handling; stable empty render; deterministic hooks not conflated with citations.

**Phase U4 (2026-04-19):** `tests/test_workbench_u4.py` — logic/audit panel empty state vs populated assumptions/calculations/checks; M5 preliminary badge and disclaimer; deterministic rows use non-citation badges; audit + retrieval response coexist; chat without session redirects.

**Phase U3 (2026-04-18):** `tests/test_local_assist_u3.py` — model-disabled path matches R1 `answer_text`; enabled + stub preserves citations/evidence/refusals boundaries; insufficient evidence and governance conflict do not apply U3 synthesis; normative vs supporting `authority_class` preserved; deterministic hooks vs document citations; unavailable provider / explicit injection; assumptions not invented. `tests/test_workbench_u3.py` — chat and evidence always show U3 synthesis control (disabled when server off); enabled when env set; POST passthrough; assist provenance/citations unchanged.
...
**Phase G5 — Derived knowledge (2026-04-18):** `tests/test_derived_knowledge_g5.py` — deterministic regeneration from governed active corpus; `document_id` / `fragment_id` anchors; bundle version bumps only when fingerprint changes; idempotent regen when corpus unchanged; canonical JSON stable ordering; missing `derived_knowledge/` returns `None`; formula registry entries remain `non_authoritative`; **retrieval + `LocalAssistOrchestrator` unchanged** after `regenerate`; explicit projection limits digests to authoritative ids only.

**Phase R2B — Reasoning bridge (2026-04-18):** `tests/test_reasoning_bridge_r2b.py` — `ReasoningBridgeService.analyze` returns structured interpretation + gaps; vertical-slice vs unknown scope; derived formula candidates linked to fragments; M5 execution status bounded; deterministic rows labeled non-normative; JSON schema validation + round-trip codec; canonical dict stable; **`LocalAssistOrchestrator` and retrieval unchanged** when bridge runs separately.

**Phase U5 (2026-04-19):** `tests/test_workbench_u5.py` — `/workbench/project/canvas` session gating; empty `q` empty-state form; with normative corpus + derived bundle + query renders interpretation / formulas / tier CSS / fragment links; execution section labels deterministic as not normative evidence; chat includes canvas nav; view-model helper round-trip on empty `ReasoningBridgeResult`.

**Phase U6 (2026-04-19):** `tests/test_workbench_u6.py` — chat/evidence/canvas include secondary-to-workflow strip + workflow URL; workflow page includes primary-surfaces return nav and secondary framing; hub orders primary vs secondary sections; session `project_id` visible on workflow; corpus includes primary nav. **Does not** change retrieval or orchestrator routes.

**Integrated case flow hardening (2026-04-19):** `tests/test_integrated_case_flow.py` — POST assist stores last query and GET evidence prefills; explicit `?q=` overrides session on chat; workflow page’s chat link includes encoded `q` when session has query; creating a new project clears stale assist query from handoff; no-session GET on chat/evidence/canvas/workflow → `303` to hub; `surface_href` / `build_case_nav` unit checks. **Does not** assert retrieval/governance changes. See `docs/implementation/PHASE_INTEGRATED_CASE_FLOW_STATUS.md`.

**Evidence original source viewer (2026-04-19):** `tests/test_evidence_original_source_viewer.py` — citation page is source-oriented (`evidence-source-view`); PDF ingest from committed `tests/fixtures/sample_text.pdf` shows PDF side-by-side + page fragment; verified file route returns `application/pdf`; integrity failure → `404`; missing original file → `fragment_only` fallback with excerpt; no-session → `303` on file route; unknown document → `404`; view-model `page_range` label; `pdf_open_url` suffix matches stored physical page; `tests/test_evidence_pdf_pages.py` — `#page=` fragment helper (1-based).
